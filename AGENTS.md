...
3. **12개 장 구조 (v3.1, 고객 주문 여정)** — 꽃배달이 처음이라면 → 상품별 특성 알아보기 → 상황별 상품 추천 → 꽃말과 색상의 의미 → 기념일 캘린더 → 리본·카드 메시지 문구 → 주문하고 결제하기 → 시간 맞춰 배송 받기 → 받은 꽃 오래 보기 → 취소·환불·품질 보상 → 자주 묻는 질문 50선 → 꽃배달 용어 사전. (총 45개 아티클)
